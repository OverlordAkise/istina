...
func TestDarkRP(t *testing.T) {
	...
	err = db.Get(&rpplayer, "SELECT * FROM luaplayer WHERE steamid != 'BOT'")
	if err != nil {
		t.Fatal(err)
	}
...
func TestJobstatsAddition(t *testing.T) {
	r := SetupRouter()
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/darkrpstat", jobstatbody)
	r.ServeHTTP(w, req)
	if w.Code != 200 {
		t.Fatal("/ had http code != 200")
	}
	...
	//Checking jobstats switches
	err = db.Get(&nv, "SELECT switchedto as value FROM jobstats WHERE jobname = 'Medic'")
	if err != nil {
		t.Fatal(err)
	}
	if nv.Value != 2 {
		t.Fatal("Jobstats switchedto in DB is not the same as input!")
	}
}
